...
<? foreach($ligadatei as $liga){?>
  ...
  if (isset($_SESSION['lmouserok']) && $_SESSION['lmouserok']>0) {?>
      <td class="nobr" align="left"><?=$liga['file_name']?> &nbsp;</td><?
			...
			$spieltagsname = spieltagsname_arr($liga['file_name']);//DH SPIELTAGSNAME
			$dh_st = $liga['aktueller_spieltag'];
			if (is_array($spieltagsname) AND is_array($spieltagsname[$dh_st])){//DH SPIELTAGSNAME
					echo $spieltagsname[$dh_st][2];
				  }//DH SPIELTAGSNAME
			else {
				echo $liga['rundenbezeichnung']." ".$liga['aktueller_spieltag'];
			}
			?> &nbsp;</td>
